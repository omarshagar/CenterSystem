...
namespace CourseSystem
{
    ...
    public partial class courses_not_enrollment_form : Form
    {
        ...
        // to do 
        int stud_id;
        public courses_not_enrollment_form(int myid)
        {
            stud_id = myid;
            InitializeComponent();
        }
        ...
        private void populateitems()
        {
            ...
            data2 = retreve_data_from_database_onebind("select course_id from enrollment where studnet_id=:stud_id", this.stud_id.ToString());
            ...
            for (int i = 0; i < id_courses_not.Count(); i++)
            {
                ...
                data4 = retreve_data_from_database_onebind("select firstname from instructor where INSTRucTOR_ID=:instructor_id_firstname ", instructor_id_firstname);
                // get lastname of instructor
                string instructor_id_lastname = data3.Rows[0][1].ToString();
                DataTable data5 = new DataTable();
                data5 = retreve_data_from_database_onebind("select lastname from instructor where INSTRucTOR_ID=:instructor_id_lastname ", instructor_id_lastname);
                string name_of_doctor = "Dr:" + data4.Rows[0][0].ToString() + ' ' + data5.Rows[0][0].ToString();

                // display using shap of usercontrol1
                UserControlStud[] listItems = new UserControlStud[id_courses_not.Count()];
