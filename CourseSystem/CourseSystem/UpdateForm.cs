...
namespace CourseSystem
{
    public partial class UpdateForm : Form
    {
        ...
        OracleCommand cmd;
        public UpdateForm(int id, char op)
        {
            option = op;
            myid = id;
            InitializeComponent();
        ...
        private void UpdateForm_Load(object sender, EventArgs e)
        {
            ...
            else if (option == 'i')
            {
                ...
                cmd = new OracleCommand();
                cmd.Connection = con;
                cmd.CommandText = "select firstname, lastname, mobile_number, password from instructor where instructor_id = :id";
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.Add("id", myid);
                OracleDataReader dr = cmd.ExecuteReader();
                if (dr.Read())
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (cont)
            {
                ...
                else if (option == 'i')
                {
                    ...
                    cmd.Connection = con;
                    string[] names = textBox1.Text.Split(' ');
                    cmd.CommandText = "update instructor set firstname =  :first, lastname = :second, mobile_number = :mob, password = :pass where instructor_id = :id";
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.Add("first", names[0]);
                    cmd.Parameters.Add("last", names[1]);
                    cmd.Parameters.Add("mob", textBox2.Text);
