    ...
    public partial class Enrollment_form : Form
    {
       ...
       */
        ...
        public void add_data_while_enroll(string name_of_dr, string name_of_course)
        {
            ...
            int i = 3;
            for (; i < name_of_dr.Length && name_of_dr[i] != '_'; i++) { firstname += name_of_dr[i]; }
            i++;
            for (; i < name_of_dr.Length && name_of_dr[i] != '_'; i++) lastname += name_of_dr[i];
             ...
             */
            DataTable dt = new DataTable();
            dt = retreve_data_from_database_threebind("select instructor_id from course, instructor where instructor.firstname =:first and instructor.lastname =:second and course.course_name =:course ", firstname, lastname, name_of_course);
            adapter.Fill(dt);
